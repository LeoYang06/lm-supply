...
public sealed class BinaryDownloader : IDisposable
{
            ...
            try
            {
                ...
                if (!entry.Sha256.StartsWith("placeholder", StringComparison.OrdinalIgnoreCase))
                {
                    if (!await VerifyChecksumAsync(targetPath, entry.Sha256, cancellationToken))
                    {
                        ...
                        throw new LocalAIException($"Checksum verification failed for {entry.FileName}");
                    }
                }
    ...
    {
        var tempPath = Path.Combine(Path.GetTempPath(), $"localai_{Guid.NewGuid()}{GetArchiveExtension(entry.Url)}");
        ...
        throw lastException is not null
            ? new LocalAIException($"Failed to download {entry.FileName} after {_options.MaxRetries + 1} attempts", lastException)
            : new LocalAIException($"Failed to download {entry.FileName} after {_options.MaxRetries + 1} attempts");
    }
    ...
    private static string GetMutexName(RuntimeBinaryEntry entry)
    {
        ...
        return $"Global\\LocalAI_Download_{hash}";
    }
