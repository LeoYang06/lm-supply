...
public sealed class ManifestProvider : IDisposable
{
    ...
    private const string DefaultManifestUrl = "https://raw.githubusercontent.com/iyulab/local-ai/main/runtime-manifest.json";
    ...
    private static HttpClient CreateDefaultHttpClient()
    {
        ...
        client.DefaultRequestHeaders.UserAgent.ParseAdd("LocalAI/1.0");
    ...
    private static string GetDefaultCacheDirectory()
    {
        ...
        if (!string.IsNullOrEmpty(hfCache))
            return Path.Combine(hfCache, "localai");

        var hfHome = Environment.GetEnvironmentVariable("HF_HOME");
        if (!string.IsNullOrEmpty(hfHome))
            return Path.Combine(hfHome, "hub", "localai");

        var xdgCache = Environment.GetEnvironmentVariable("XDG_CACHE_HOME");
        if (!string.IsNullOrEmpty(xdgCache))
            return Path.Combine(xdgCache, "huggingface", "hub", "localai");

        // Platform-specific defaults
        if (OperatingSystem.IsWindows())
        {
            var localAppData = Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData);
            return Path.Combine(localAppData, "huggingface", "hub", "localai");
        }
        else if (OperatingSystem.IsMacOS())
        {
            var home = Environment.GetFolderPath(Environment.SpecialFolder.UserProfile);
            return Path.Combine(home, "Library", "Caches", "huggingface", "hub", "localai");
        }
        else
        {
            var home = Environment.GetFolderPath(Environment.SpecialFolder.UserProfile);
            return Path.Combine(home, ".cache", "huggingface", "hub", "localai");
        }
    }
